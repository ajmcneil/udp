...
test_that("sheval() matches the piecewise definition", {
  s <- shuffle(c(2, 1, 3)) # swap the first two thirds, third fixed
  expect_equal(sheval(s, c(0.1, 0.2)), c(0.1, 0.2) + 1 / 3)
  expect_equal(sheval(s, c(0.4, 0.6)), c(0.4, 0.6) - 1 / 3)
  expect_equal(sheval(s, c(0.7, 0.9)), c(0.7, 0.9))

  sf <- shuffle(c(1, 2, 3), signs = c(-1, 1, 1)) # flip the first third
  expect_equal(sheval(sf, c(0.1, 0.25)), 1 / 3 - c(0.1, 0.25))

  expect_equal(sheval(s, c(0, 1)), c(1 / 3, 1))
})

test_that("sheval() maps [0, 1] onto [0, 1] and preserves attributes", {
  set.seed(1)
  for (rep in seq_len(20)) {
    m <- sample.int(8, 1)
    s <- shuffle(sample.int(m), signs = sample(c(-1, 1), m, replace = TRUE))
    y <- sheval(s, seq(0, 1, length.out = 101))
    expect_true(all(y >= -1e-12 & y <= 1 + 1e-12))
  }

  u <- ts(seq(0.05, 0.95, length.out = 12), frequency = 4)
  expect_s3_class(sheval(shuffle(c(2, 1)), u), "ts")
})

test_that("shinverse() inverts sheval() in both directions", {
  set.seed(2)
  u <- sort(runif(300))
  for (rep in seq_len(25)) {
    m <- sample(2:10, 1)
    s <- shuffle(sample.int(m), signs = sample(c(-1, 1), m, replace = TRUE))
    expect_equal(shinverse(s, sheval(s, u)), u, tolerance = 1e-9)
    expect_equal(sheval(s, shinverse(s, u)), u, tolerance = 1e-9)
  }
})
...
test_that("shuffles preserve the uniform distribution", {
  ...
  for (perm in list(c(2, 1, 3), c(4, 2, 1, 3), c(1, 2, 3, 4, 5))) {
    ...
    p <- suppressWarnings(stats::ks.test(sheval(s, u), "punif")$p.value)
    expect_gte(p, 0.01)
  }
})
